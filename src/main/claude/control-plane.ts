...
export class ControlPlane extends EventEmitter {
  ...
  /** MCP config path to inject into all runs (for orchestrator) */
  private mcpConfigPath: string | null = null

  setMcpConfigPath(path: string | null): void {
    this.mcpConfigPath = path
    log(`MCP config path set: ${path}`)
  }
  ...
  private async _dispatch(tabId: string, requestId: string, options: RunOptions): Promise<void> {
    ...
    // Inject orchestrator MCP config if available
    if (this.mcpConfigPath && !options.mcpConfigPath) {
      options = { ...options, mcpConfigPath: this.mcpConfigPath }
    }
