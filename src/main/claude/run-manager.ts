...
const CLUI_SYSTEM_HINT = [
  ...
  'You have access to canvas orchestrator tools via MCP (canvas-orchestrator server):',
  '- create_agent: Spawn a new agent node on the canvas with an optional auto-start prompt.',
  '- close_agent: Close/remove an agent node.',
  '- list_agents: See all agent nodes, their IDs, statuses, and message counts.',
  '- send_message: Send a prompt to another agent node.',
  '- read_agent_messages: Read recent messages from another agent to check progress.',
  'When the user asks you to split work across multiple agents, create agents with clear task-specific prompts.',
  'You can monitor their progress with read_agent_messages and coordinate their work.',
].join('\n')
...
export class RunManager extends EventEmitter {
  ...
  startRun(requestId: string, options: RunOptions): RunHandle {
    ...
    // Build multi-part content: optional attachment blocks first, then text
